...
'''
stats_2p_2u.py
This script performs a permutation test for CR across CS2U and CS2P
'''
...
def main(args):
  ...
  def ttest_w_bs(p1,p2,ntrials, nbs = 10000):
    if any(ntrials == 0): return np.nan,np.nan
  ...
  def ttest_ind_bs(p1,p2,n1,n2, nbs = 65536):
    if any(n1 == 0) or any(n2 == 0): return np.nan,np.nan
    sqr_nbs = int(nbs**.5)
  ...
  # files to process
  os.chdir(args._in)
  for stat_type in ['cr','onset']:
    f_p = []; f_u = []
    for f in os.listdir():
      if fnmatch(f,'*pr_2pair*') and fnmatch(f,f'*{stat_type}*'): f_p = f
      if fnmatch(f,'*pr_2unp*') and fnmatch(f,f'*{stat_type}*'): f_u = f
    
    master_log = open(f'{args.out}{stat_type}.log','w')
    
    df_p = pd.read_table(f_p, index_col = 0)
    df_u = pd.read_table(f_u, index_col = 0)
    
    if stat_type == 'cr':
      datadf_p = df_p[['total_cr','first_third_cr','second_third_cr','last_third_cr']]
      datadf_u = df_u[['total_cr','first_third_cr','second_third_cr','last_third_cr']]
      ndf_p = df_p['trial_count'].to_numpy()
      ndf_p = pd.DataFrame(dict(trial_count = ndf_p, trials_1 = ndf_p/3, 
                                trials_2 = ndf_p/3, trials_3 = ndf_p/3))
      ndf_u = df_u['trial_count'].to_numpy()
      ndf_u = pd.DataFrame(dict(trial_count = ndf_u, trials_1 = ndf_u/3, 
                                trials_u = ndf_u/3, trials_3 = ndf_u/3))
    else:
      datadf_p = df_p[['mean_onset','first_third_onset','second_third_onset','last_third_onset']]
      datadf_u = df_u[['mean_onset','first_third_onset','second_third_onset','last_third_onset']]
      ndf_p = df_p[['crcount','first_third_crcount','second_third_crcount','last_third_crcount']]
      ndf_u = df_u[['crcount','first_third_crcount','second_third_crcount','last_third_crcount']]
    ...
    for stage, i in zip(datadf_p.columns,range(4)):
      stat_p = datadf_p[stage].to_numpy()
      stat_u = datadf_u[stage].to_numpy()
      
      n_p = ndf_p.iloc[:,i].to_numpy(); n_u = ndf_u.iloc[:,i].to_numpy()
      
      ntrials = (n_p * n_u)**.5 
      # weight by the geometric mean of #trials because the number of trials may 
      # be vastly different across paired and unpaired
      p_all,t_all = ttest_w_bs(stat_p,stat_u,ntrials)
      p_aware,t_aware = ttest_w_bs(stat_p[aware],stat_u[aware],ntrials[aware])
      ...
      # weighted mean
      w_mean_p, w_std_p = wmean_std(stat_p, n_p)
      w_mean_u, w_std_u = wmean_std(stat_u, n_u)
      ...
      print(f'observed weighted mean for paired: {w_mean_p:.3f} +/- {w_std_p:.3f}', file = master_log)
      print(f'observed weighted mean for unpaired: {w_mean_u:.3f} +/- {w_std_u:.3f}', file = master_log)
      
      # plot figure
      c_p = '#c00000'; c_u = '#0070c0'
      
      _,ax = plt.subplots(figsize = (3,4))
      for i in range(len(stat_p)):
        ax.plot([1,2], [stat_p[i],stat_u[i]], marker = '.', color = '#dfdfdf')
      ax.errorbar([1],w_mean_p, yerr = w_std_p, color = c_p, ecolor = c_p)
      ax.errorbar([2],w_mean_u, yerr = w_std_u, color = c_u, ecolor = c_u)
      ax.bar([1],[w_mean_p], color = c_p, edgecolor = c_p, fill = False)
      ax.bar([2],[w_mean_u], color = c_u, edgecolor = c_u, fill = False)
      ax.set_xticks([1,2], ['CS2P','CS2U'])
      ...
      stat_pa = stat_p[aware]; stat_ua = stat_u[aware]
      w_mean_pa, w_std_pa = wmean_std(stat_pa, ntrials[aware])
      w_mean_ua, w_std_ua = wmean_std(stat_ua, ntrials[aware])
      _,ax = plt.subplots(figsize = (3,4))
      for i in range(len(stat_pa)):
        ax.plot([1,2], [stat_pa[i],stat_ua[i]], marker = '.', color = '#dfdfdf')
      ax.errorbar([1],w_mean_pa, yerr = w_std_pa, color = c_p, ecolor = c_p)
      ax.errorbar([2],w_mean_ua, yerr = w_std_ua, color = c_u, ecolor = c_u)
      ax.bar([1],[w_mean_pa], color = c_p, edgecolor = c_p, fill = False)
      ax.bar([2],[w_mean_ua], color = c_u, edgecolor = c_u, fill = False)
      ax.set_xticks([1,2], ['CS2P','CS2U'])
    ...
    ctrl_df.to_csv(f'{args.out}{stat_type}_ctrlvars.log', sep = '\t', index = True,
                   header = True)
    # contrast
    diff = datadf_p.iloc[:,1:].to_numpy() - datadf_u.iloc[:,1:].to_numpy()
...
if __name__ == '__main__':
  ...
  parser.add_argument('-o','--out',dest = 'out', default = '../stats/2p_2u',
